...
- El select de "quién pagó" es `components/shared/payer-select.tsx`, y los campos que de ahí salen para `expenses` los arma `camposDePagador()` (`lib/utils/payer.ts`). No rehacerlo a mano en un formulario nuevo: cuando cada uno de los cuatro formularios que cargan gastos tenía su copia, agregar "Pagamos los dos" en uno dejó a los otros tres sin la opción.
...
- Borrar un gasto **devuelve a "pendiente" la cuota de gasto fijo que pagaba** (trigger `expenses_reset_instancia`, migración 0012). Antes no: la FK `expense_id` es `on delete set null` y limpiaba el vínculo dejando `status` en `'paid'`, así que quedaban cuotas pagadas sin nada detrás. La restricción `recurring_instance_pagada_tiene_gasto` hace que ese estado ya no se pueda escribir. Si algún día agregás otro camino que marque una cuota como pagada, tiene que crear el gasto primero.
